...
class Predictor:
    ...
    def open_Quotes_setting(self, ref_trade_price, target_trade_price,timestamp,date_time):
        ...
        print(f'reference_price = {ref_trade_price * (1 + self.slippage_number(slippage,self.table["w1"]))} . size = {abs(self.ref_size)} , side = {self.side_determination(self.ref_size)}')
        print(f'target_price = {target_trade_price *(1 + self.slippage_number(slippage,self.table["w2"]))} . size = {abs(self.target_size)} , side = {self.side_determination(self.target_size)}')
    ...
    def draw_pictrue(self,open_threshold,stop_loss_threshold,stamp,POS,trade_time):
        if self.drawpic :
            ...
            if POS == 'topopen' or POS == 'downopen':
                plt.savefig(path + str(self.ref_symbol) + '_' + str(self.target_symbol) + '_' +POS+'spread_'+str(trade_time)+f'_{self.config.period}.png')
            elif POS == 'close':
                plt.savefig(path + str(self.ref_symbol) + '_' + str(self.target_symbol) + '_' +POS+'spread_'+ str(trade_time)+f'_{self.config.period}.png')
        plt.close()
